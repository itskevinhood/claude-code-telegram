...
| `src/triage/` | Alert triage (self-heal Phase 3): queue worker, read-only tool policy, runner, store (`data/triage.db`) |
...
| Why an alert got (or didn't get) a 🩺 Triage reply | `data/triage.db` (`alert_triage.status`), `journalctl -u claude-telegram-bot \| grep -i triage`; gates in `docs/self-heal-roadmap.md` Phase 3 |
